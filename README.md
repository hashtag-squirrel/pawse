...
|                                        | As a user, I want to see the cats living in the cafe so that I can familiarize myself with them before my visit               |
...
|                                        | As a user, I want to have easy access to the social media accounts of the cafe so that I can engage with the owners and other customers   |
...
|                                        | As a site admin, I want to be able to adjust the menu from the front end so that it is always up to date                               |
|                                        | - As a site admin, I want to be able to add new menu items from the front end                                                          |
|                                        | - As a site admin, I want to be able to edit menu items from the front end                                                             |
|                                        | - As a site admin, I want to be able to delete menu items from the front end                                                           |
|                                        | As a site admin, I want to be able to adjust the kittens from the front end so that customers always see the current kittens           |
|                                        | - As a site admin, I want to be able to add new cats from the front end                                                                |
|                                        | - As a site admin, I want to be able to edit cats from the front end                                                                   |
|                                        | - As a site admin, I want to be able to delete cats from the front end                                                                 |
...
Additional functionalities can be accessed from the nav bar or as subpages, depending on whether a user is signed in or not and depending on the user's role. 
...
The logo design is based on the name of the cafe, which is a pun made of the paws of a cat and pause like a break. Therefore, the logo is a stylized cat paw with a pause sign on the big circle. 
...
Here is the updated version, reflecting the current state of the models:

![ERD new](docs/pawse-erd-new.png)

The User model is provided by Django.

The MenuCategory and MenuItem models are utilized to display the menu list divided by category and hold information about the price of the different items as well. An update made during development was to add the "order" field on the MenuCategory model, which is used to display the categories in a specified order on the website. 
...
To keep best track of the time needed for the tickets, I created a [Roadmap](https://github.com/users/hashtag-squirrel/projects/2/views/2?groupedBy%5BcolumnId%5D=Milestone) displaying the milestones and their respective tickets, which is shown here:
...
The goal of the planning was to even out the work I had to do over the sprints and when I generated a chart with Story Points per Sprint, I can see that it worked out quite well. Sprint 6 was expected to have more story points since it lasted about twice as long as other sprints. 
...
Desktop view with an active link (Pawse) and hovered link (Menu):
...
The Account pages are all designed similarly, with a simple card including a form (Login/Register) or a confirmation button (Logout). They adhere to the main design of the page and have a bright yellow button on all pages as a pop of color. 

The Registration does not require an email address, because I wanted to avoid collecting personal data from users. This is also the reason why there is no option to change the password via a "Forgot Password" link since this would require an email address. 
...
The description is shortened on mobile devices to not cover too much of the screen.
...
It is separated from the testimonials by a 25% width horizontal rule as a visual divider. It has a headline and then features an embedded map that spans the full viewport width. The map has a pointer on Stockholm since this is the city I live in and the cafe doesn't exist. 
...
The menu items show a name highlighted in a darker shade of yellow for readability, the price, and a short description. Menu items can also be managed by the site admin in the backend. 
...
On mobile, it shows one card per row:
...
Below the carousel, authenticated users can also keep track of their applications and their details of them, in the same style as other sections before. Here they can also click a button to edit or delete the applications. 
...
Additionally, to more content that could be added to the existing pages, many features can be added to flesh out the project. Here is a list of potential features that can be added with varying amounts of work:
...
- The possibility of deleting one's account
...
- Balsamiq - for the creation of the wireframes
...
1. Log in to the Heroku dashboard and create a new app.
...
4. Create a virtual environment by typing `pipenv shell` in your terminal
5. Install the required dependencies specified in the Pipfile by typing `pipenv install` in the terminal.
6. Note: The project is set up to use environment variables. You will need to set these up in your local environment.
...
Text content for the following sections was generated by OpenAIs ChatGPT-3.5 (July 20 version):
...
- [Redirect to the same page after login/logout/register](https://stackoverflow.com/a/13595154)
